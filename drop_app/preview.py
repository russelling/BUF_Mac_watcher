...
def shot_cdl_path(episode: str, sequence: str, shot: str) -> Optional[str]:
    ...
    ../scripts/qt_bake_oiio.py -- if that naming convention changes again,
    update both, so Preview's CDL never silently diverges from what the
    actual bake applies.

    Added 2026-08-17: shots now name their CDL {shot}_{layer}_v{version}.cc
    ...
    the highest version when more than one matches. Falls back to the older
    {shot}_BG1.cdl / {shot}.cdl / {shot}.cc candidates for anything that
    predates the versioned naming.
    """
    ...
    pattern = re.compile(r"^%s_(?P<layer>[^_]+)_v(?P<version>\d+)\.cc$" % re.escape(shot))
    candidates = []
    ...
    if candidates:
        candidates.sort()
        return os.path.join(plates_dir, candidates[-1][2])
